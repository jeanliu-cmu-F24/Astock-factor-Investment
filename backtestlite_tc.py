...
class Backtestlite_tc(object):
    def __init__(self, factor_path, direction, fwdrtn, mkt_index, save_path_factor, cycle, start, end, quantile, statuslimit, tc):
    ...
    def cal_turnover(self):
        ...
        self.turnover_sum = self.turnover.abs().sum(axis = 1)
        self.turnover = (100*self.turnover_sum / self.net_value).iloc[1:]

    def cal_net_value_tc(self):
        self.net_value_tc = self.net_value - self.tc * self.turnover_sum
        return

    def cal_ls_rtn_tc(self):
        self.ls_rtn_tc = self.net_value_tc/self.net_value_tc.shift(1) - 1
        return
